...
<section class="layout"><div><div class="grid2"><section class="panel"><h2>Strategy Equity</h2><div class="chartBox"><canvas id="equityChart" class="chart"></canvas></div></section><section class="panel"><h2>Daily PnL Pulse</h2><div class="chartBox"><canvas id="dailyChart" class="chart"></canvas></div></section></div><section class="panel" style="margin-top:12px"><h2>Latest Signal Tape</h2><div id="scanTable"></div></section><section class="panel" style="margin-top:12px"><h2>Trade Blotter</h2><div id="tradeTable"></div></section></div>
<aside><section class="panel"><h2>My Recommendation</h2><div id="recommendations" class="rec"></div></section><section class="panel" style="margin-top:12px"><h2>Market Heatmap</h2><div id="heatmap" class="heat"></div></section><section class="panel" style="margin-top:12px"><h2>Symbol Edge Board</h2><div class="chartBox"><canvas id="symbolChart" class="chart"></canvas></div></section></aside></section>
...
$("cards").innerHTML=[card("USDT Balance",money(s.balance_usdt),idr(s.balance_idr)),card("Growth",pct(s.growth_pct),"Target runway"),card("Cycle","#"+(s.heartbeat_cycle??"-"),"Heartbeat "+(s.heartbeat_age_sec??"-")+"s"),card("Signal Feed",(s.scan_rows||[]).length,"Actual scan rows"),card("Market Feed",(s.market||[]).length,"Binance 24h pairs"),card("Closed PnL",`<span class="${cls(t.pnl)}">${money(t.pnl)}</span>`,(t.count||0)+" closed trades")].join("");
...
try{renderShell();makeCharts()}catch(e){document.body.insertAdjacentHTML("afterbegin",`<div style="position:sticky;top:0;z-index:9;background:#ea3943;color:white;padding:10px;font-family:Consolas">Dashboard render error: ${String(e.message||e)}</div>`)}setTimeout(()=>location.reload(),30000);
