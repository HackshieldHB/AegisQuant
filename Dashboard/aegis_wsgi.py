...
</style></head><body><div class="appShell"><aside class="sidebar"><div class="logo"><strong>AegisQuant</strong><span>Automated trading</span></div><nav class="tabs"><button class="tab active" data-view="overview">Overview</button><button class="tab" data-view="markets">Markets</button><button class="tab" data-view="signals">Signals</button><button class="tab" data-view="trades">Trades</button><button class="tab" data-view="system">System</button></nav><div class="sideFoot">Live production<br>Binance Spot</div></aside><main class="main"><header class="topbar"><div><h1 id="pageTitle">Overview</h1><p>Portfolio, market and execution monitoring</p></div><div class="statusLine"><span id="healthBadge" class="badge">LOADING</span><span id="modelBadge" class="badge">MODEL</span><span id="updatedAt" class="muted"></span></div></header><div class="shell">
...
<section id="markets" class="view"><div class="grid2"><section class="panel"><h2>Market overview</h2><div id="heatmap" class="heat"></div></section><section class="panel"><h2>24h performance</h2><div class="chartBox"><canvas id="symbolChart" class="chart"></canvas></div></section></div><section class="panel" style="margin-top:14px"><h2>Market details</h2><div id="marketTable"></div></section></section>
...
$("marketTable").innerHTML=table(["Asset","Last price","24h","High","Low","Quote volume"],(s.market||[]).map(m=>`<tr><td><b>${esc(m.symbol)}</b></td><td>${Number(m.price).toLocaleString("en-US",{maximumFractionDigits:m.price<1?8:2})}</td><td class="${cls(m.change)}">${pct(m.change)}</td><td>${Number(m.high).toLocaleString("en-US",{maximumFractionDigits:m.high<1?8:2})}</td><td>${Number(m.low).toLocaleString("en-US",{maximumFractionDigits:m.low<1?8:2})}</td><td>$${Number(m.volume||0).toLocaleString("en-US",{maximumFractionDigits:0})}</td></tr>`),"Live market data unavailable.");
