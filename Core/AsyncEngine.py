...
from Data.Labeling.TripleBarrier import cusum_filter
...
def is_cusum_event(df, pct: float = 0.01, window: int = 150) -> bool:
    """Return True if the most recent bar in `df` is a CUSUM event.

    Train/serve alignment: models are trained only on CUSUM-sampled event bars
    (trainer uses h = mean(close) * 0.01). We replay the same symmetric CUSUM
    filter over the most recent `window` bars and check whether the latest bar
    is flagged. Off-event bars are out-of-distribution for the model, so an
    AI-only entry on such a bar is untrustworthy. Fail-open (return True) on any
    error so a computation problem never silently blocks all trading.
    """
    try:
        if df is None or len(df) < 5 or "close" not in df.columns:
            return True
        recent = df.tail(int(window))
        avg_price = float(recent["close"].mean())
        if not avg_price or avg_price <= 0:
            return True
        h = avg_price * float(pct)
        events = cusum_filter(recent, h=h)
        if events is None or len(events) == 0:
            return False
        return recent.index[-1] in events.index
    except Exception:
        return True
...
class AsyncEngine:
    ...
    async def _process_symbol(
        ...
        try:
            ...
            # AI-only entries are only trusted on CUSUM event bars, matching the
            # distribution the model was trained on. TA-confirmed signals bypass.
            cusum_required = bool(gates_cfg.get("CUSUM_EVENT_REQUIRED", True))
            cusum_ok = True
            if cusum_required and base_signal == "HOLD" and ai_signal != "HOLD":
                cusum_ok = is_cusum_event(
                    df,
                    float(gates_cfg.get("CUSUM_EVENT_PCT", 0.01)),
                    int(gates_cfg.get("CUSUM_WINDOW_BARS", 150)),
                )
            trace["cusum_event"] = cusum_ok

            ai_only_conf = float(gates_cfg.get("AI_ONLY_DIRECTIONAL_CONF", 0.74))
            hold_reason = "No actionable signal"
            if base_signal == "HOLD" and (ai_confidence < ai_only_conf or not cusum_ok):
                signal = "HOLD"
                support = bull_indicators if ai_signal == "BUY" else bear_indicators
                if not cusum_ok:
                    hold_reason = (
                        f"No CUSUM event (off-distribution bar) — AI-only entry suppressed "
                        f"(conf={ai_confidence:.0%}, mass={directional_mass:.0%})"
                    )
                else:
                    hold_reason = (
