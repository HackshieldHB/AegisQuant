...
SIGNAL_GATES = {
    ...
    # distribution, so AI-only entries are only trusted when the current bar is
    # itself a CUSUM event. TA-confirmed signals are unaffected.
    "CUSUM_EVENT_REQUIRED": str(os.getenv("CUSUM_EVENT_REQUIRED", "True")).lower() == "true",
    "CUSUM_EVENT_PCT": float(os.getenv("CUSUM_EVENT_PCT", "0.01")),  # matches trainer h = avg_price*0.01
    "CUSUM_WINDOW_BARS": int(os.getenv("CUSUM_WINDOW_BARS", "150")),  # warm-up window for the accumulator
}
